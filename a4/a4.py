...
def draw_spiral(w, side, ang, n, sp): #done
    """
    ...
    assert is_valid_iteration(n), report_error('invalid # of iterations',n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    #check ang precondition
    assert type(ang)==int or type(ang)==float, report_error('invalid angle',ang)
...
def draw_spiral_helper(t, side, ang, n, sp): #done
    """
...
def radiate_petals(w, radius, width, n, sp): #done
    """
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(radius), report_error('invalid radius length',radius)
    assert is_valid_length(width),  report_error('invalid width',width)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    #assert n precondition
    assert (type(n) == int and n >= 2), report_error('invalid # of lines', n)
...
def radiate_petals_helper(t, radius, width, n, sp): #done
    """
    ...
    assert is_valid_length(radius), report_error('invalid radius length',radius)
    assert is_valid_length(width),  report_error('invalid width',width)
    assert (type(n) == int and n >= 2), report_error('invalid # of petals',n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    ...
    for i in range(n):
        ...
        #if i is even, open petal
        if i % 2 == 0:
            draw_diamond(t, radius, width)
        #if i is odd, closed petal
        else:
            t.forward(radius)
            t.backward(radius)
        t.right(ang)
...
def triangle_helper(p, x, y, side, d): #done
    """
    ...
    assert type(x) == int or type(x) == float, report_error('x is invalid #', x)
    assert type(y) == int or type(y) == float, report_error('y is invalid #', y)
    assert is_valid_length(side), report_error('Invalid side length', side)
    assert is_valid_depth(d), report_error('Invalid depth', d)

    # Hint: Use fill_triangle to draw an individual triangle
    #base case: depth 0 (draw just a triangle)
    if d == 0:
        fill_triangle(p, x, y, side)
    #recursive case: arrange three of the previous depth into a triangle shape
    else:
        #center 1 (bottom left)
        x1 = x - side/4
        y1 = y - side*(math.sqrt(3))/8
        triangle_helper(p, x1, y1, side/2.0, d-1)
        #center 2 (bottom right)
        x2 = x + side/4
        y2 = y - side*(math.sqrt(3))/8
        triangle_helper(p, x2, y2, side/2.0, d-1)
        #center 3 (top)
        x3 = x
        y3 = y + side*(math.sqrt(3))/8
        triangle_helper(p, x3, y3, side/2.0, d-1)
...
def island(w, side, d, sp): #done
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('invalid side length', side)
    assert is_valid_depth(d), report_error('depth is not a valid value', d)
    assert is_valid_speed(sp), report_error('speed is not a valid value', sp)
    # HINT: w.clear() clears window.
    w.clear()
    # Create a turtle and draw
    t = Turtle(w)
...
def island_edge(t, side, d): #done
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('side is an invalid value', side)
    assert is_valid_depth(d), report_error('depth is not a valid value', d)
